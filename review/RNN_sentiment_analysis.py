...
sequence_len = 50 # try 50 and 200 also
print('sequence_len: %3d'% sequence_len)

model = RNN_model(vocab_size,no_hidden_units) # try 300 as well
...
for epoch in range(no_of_epochs):
    ...
    if((epoch+1)%2)==0:
        ...
        for i in range(0, L_Y_test, batch_size):
            ...
            sequence_length = sequence_len # try 200 also
            x_input = np.zeros((batch_size,sequence_length),dtype=np.int)
            for j in range(batch_size):
                x = np.asarray(x_input2[j])
                sl = x.shape[0]
...
torch.save(model,'rnn_languageModel.model')
data = [train_loss,train_accu,test_accu]
data = np.asarray(data)
np.save('data_languageModel.npy',data)
